...
local function setupAntiCheatBypass()
    ...
    -- Bypass: neutralizes weak-table garbage collection checks from specific anticheat modules
    local originalSetmetatable = getrenv().setmetatable
    local metatableHook
    metatableHook = hookfunction(originalSetmetatable, newcclosure(function(object, metatable)
        if metatable and type(metatable) == "table" then
            local mode = rawget(metatable, "__mode")
            if (mode == "kv" or mode == "v" or mode == "k")
                and (debug.traceback():find("MiscellaneousController")
                    or debug.traceback():find("CameraSecurity")
                    or debug.traceback():find("AnalyticsPipelineController")) then
                return metatableHook({ 1, 2, 3 }, {})
            end
        end
        return metatableHook(object, metatable)
    end))

    -- Bypass: prevents the anticheat from kicking the local player
...
if setupAntiCheatBypass then
    print("Anticheat bypass setup successfully.")
else
    print("Failed to set up anticheat bypass.")
end
task.wait(5)
...
local Window = Library:CreateWindow({
    Title = "ph4ntms | " .. game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name,
    Center = true, 
    AutoShow = true,
    Resizable = true,
...
-- tabs (combat + character added, Visuals/World/Misc/Settings reused)
local Tabs = {
...
local Tab2 = Tabs.Visuals
local Tab6 = Tabs['UI Settings']

-- Sense ESP (sirius.menu/sense)
local function loadSense()
	local Sources = {
		"https://sirius.menu/sense",
		"https://raw.githubusercontent.com/jensonhirst/Sirius/request/library/sense/source.lua",
		"https://raw.githubusercontent.com/GhostDuckyy/ESP-Library/main/Sense/source.lua",
	}
	...
	error("Sense ESP failed to load from every source.")
end

local Sense = loadSense()

-- Phantom Forces: resolve the equipped weapon name for the ESP
do
	local Ok, FighterController = pcall(require, LocalPlayer.PlayerScripts.Controllers.FighterController)
	if Ok then
		Sense.getWeapon = function(player)
			...
			if Ok and Fighter and Fighter.EquippedItem then
				return Fighter.EquippedItem.Name
			end
			return "Unknown"
		end
	end
...
local EnemySettings = Sense.teamSettings.enemy
local FriendlySettings = Sense.teamSettings.friendly

local function applyToBoth(Setting)
	return function(Value)
		EnemySettings[Setting] = Value
		FriendlySettings[Setting] = Value
	end
end

local ESPGroup = Tab2:AddLeftGroupbox("ESP")

local ESPEnabled = ESPGroup:AddToggle("ESPSenseEnabled", {
	Text = "ESP Enabled",
	Default = false,
})

local ESPOptions = ESPGroup:AddDependencyBox()
ESPOptions:SetupDependencies({ { ESPEnabled, true } })

local ESPBox = ESPOptions:AddToggle("ESPBox", { Text = "Box" })
local ESPBoxFill = ESPOptions:AddToggle("ESPBoxFill", { Text = "Box Fill" })
local ESPBox3D = ESPOptions:AddToggle("ESPBox3D", { Text = "3D Box" })
local ESPHealthBar = ESPOptions:AddToggle("ESPHealthBar", { Text = "Health Bar" })
local ESPHealthText = ESPOptions:AddToggle("ESPHealthText", { Text = "Health Text" })
local ESPName = ESPOptions:AddToggle("ESPName", { Text = "Name" })
local ESPDistance = ESPOptions:AddToggle("ESPDistance", { Text = "Distance" })
local ESPWeapon = ESPOptions:AddToggle("ESPWeapon", { Text = "Weapon" })
local ESPTracer = ESPOptions:AddToggle("ESPTracer", { Text = "Tracer" })
local ESPTracerOrigin = ESPOptions:AddDropdown("ESPTracerOrigin", {
	Text = "Tracer Origin",
	Values = { "Bottom", "Middle", "Top" },
	Default = 1,
})
local ESPArrows = ESPOptions:AddToggle("ESPArrows", { Text = "Off-Screen Arrows" })
local ESPChams = ESPOptions:AddToggle("ESPChams", { Text = "Chams" })
local ESPMaxDistance = ESPOptions:AddSlider("ESPMaxDistance", {
	Text = "Max Distance",
	Min = 100,
	Max = 2000,
	Default = 500,
...
-- Wire the GUI to the Sense configuration
ESPEnabled:OnChanged(function(Value)
	EnemySettings.enabled = Value
	FriendlySettings.enabled = Value
end)

ESPBox:OnChanged(applyToBoth("box"))
ESPBoxFill:OnChanged(applyToBoth("boxFill"))
ESPBox3D:OnChanged(applyToBoth("box3d"))
ESPHealthBar:OnChanged(applyToBoth("healthBar"))
ESPHealthText:OnChanged(applyToBoth("healthText"))
ESPName:OnChanged(applyToBoth("name"))
ESPDistance:OnChanged(applyToBoth("distance"))
ESPWeapon:OnChanged(applyToBoth("weapon"))
ESPTracer:OnChanged(applyToBoth("tracer"))
ESPTracerOrigin:OnChanged(applyToBoth("tracerOrigin"))
ESPArrows:OnChanged(applyToBoth("offScreenArrow"))
ESPChams:OnChanged(applyToBoth("chams"))
ESPMaxDistance:OnChanged(function(Value)
	Sense.sharedSettings.limitDistance = true
	Sense.sharedSettings.maxDistance = Value
end)

-- Color pickers (attached to their respective toggles)
local function addColorPicker(Toggle, Setting, Default, Transparency)
	Toggle:AddColorPicker("ESPColor_" .. Setting, {
		Default = Default,
		Transparency = Transparency,
	})

	Options["ESPColor_" .. Setting]:OnChanged(function(Color, PickerTransparency)
		local NewColor = { Color, PickerTransparency }
		EnemySettings[Setting] = NewColor
		FriendlySettings[Setting] = NewColor
	end)
end

addColorPicker(ESPBox, "boxColor", Color3.fromRGB(255, 0, 0), 1)
addColorPicker(ESPBoxFill, "boxFillColor", Color3.fromRGB(255, 0, 0), 0.5)
addColorPicker(ESPBox3D, "box3dColor", Color3.fromRGB(255, 0, 0), 1)
addColorPicker(ESPName, "nameColor", Color3.fromRGB(255, 255, 255), 1)
addColorPicker(ESPTracer, "tracerColor", Color3.fromRGB(255, 0, 0), 1)
addColorPicker(ESPChams, "chamsFillColor", Color3.fromRGB(51, 51, 51), 0.5)

-- Load the ESP and clean it up when the library is unloaded
Sense.Load()
...
-- carried-over helpers (from skidhub)
...
local function instanceSafeRequire(moduleRef, timeoutSec)
    ...
    error("[skidhub] module load failed: " .. key .. " (" .. tostring(lastErr) .. ")")
end
...
local InstanceWeaponTranslations = {
    korean = {
        ["Assault Rifle"] = "ëŒê²© ì†Œì´", ["Handgun"] = "ê¶Œì´", ["Shotgun"] = "ì‚°íƒ„ì´", ["Sniper"] = "ì €ê²©ì´",
        ["Bow"] = "í™œ", ["Burst Rifle"] = "ë²„ìŠ¤íŠ¸ ì†Œì´", ["Crossbow"] = "ì„ê¶", ["Gunblade"] = "ê±´ë¸”ë ˆì´ë“œ",
        ["RPG"] = "RPG", ["Energy Rifle"] = "ì—ë„ˆì§€ ì†Œì´", ["Flamethrower"] = "í™”ì—¼ë°©ì‚¬ê¸°",
        ["Grenade Launcher"] = "ìœ íƒ„ ë°œì‚¬ê¸°", ["Minigun"] = "ë¯¸ë‹ˆê±´", ["Paintball Gun"] = "íŽ˜ì¸íŠ¸ë³¼ ê±´",
        ["Distortion"] = "ë””ìŠ¤í† ì…˜", ["Permafrost"] = "ì˜êµ¬ ë™í† ", ["Daggers"] = "ë‹¨ê²€", ["Flare Gun"] = "í”Œë ˆì–´ ê±´",
        ["Revolver"] = "ë¦¬ë³¼ë²„", ["Shorty"] = "ì‡¼í‹°", ["Spray"] = "ìŠ¤í”„ë ˆì´", ["Uzi"] = "UZI",
        ["Energy Pistols"] = "ì—ë„ˆì§€ ê¶Œì´", ["Exogun"] = "ì—‘ì†Œê±´", ["Slingshot"] = "ìƒˆì´", ["Warper"] = "ì›Œí¼",
        ["Fists"] = "ì£¼ë¨¹", ["Battle Axe"] = "ì „íˆ¬ ë„ë¼", ["Chainsaw"] = "ì „ê¸°í†±", ["Katana"] = "ì¹´íƒ€ë‚˜",
        ["Knife"] = "ë‚˜ì´í”„", ["Riot Shield"] = "ë°©íŒ¨", ["Scythe"] = "ë‚«", ["Maul"] = "ë§ì¹˜",
        ["Trowel"] = "ëª¨ì¢…ì‚½", ["Grenade"] = "ìˆ˜ë¥˜íƒ„", ["Flashbang"] = "ì„¬ê´‘íƒ„", ["Freeze Ray"] = "ëƒ‰ë™ ê´‘ì„ ",
        ["Jump Pad"] = "ì í”„ íŒ¨ë“œ", ["Molotov"] = "í™”ì—¼ë³‘", ["Satchel"] = "ê°€ë°©", ["Smoke Grenade"] = "ì—°ë§‰íƒ„",
        ["War Horn"] = "ë‚˜íŒ”", ["Medkit"] = "ì˜ë£Œ í‚¤íŠ¸", ["Substapce Tripmine"] = "ì§€ë¢°", ["Warpstone"] = "ì›Œí”„ì„",
        ["Hook"] = "ê°ˆê³ ë¦¬", ["Spear"] = "ì°½", ["Grappler"] = "ê·¸ëž˜í”ŒëŸ¬", ["Maul"] = "ë§ì¹˜",
    },
    spanish = {
...
local InstanceLocaleTable = {
    ...
    korean = {
        studs = "%.0f ìŠ¤í„°ë“œ", studs_short = "%d ìŠ¤í„°ë“œ", select_weapon = "ë¬´ê¸°ë¥¼ ë¨¼ì € ì„ íƒí•˜ì„¸ìš”",
        apply = "ì ìš©", apply_all = "ì „ì²´ ì ìš©", language = "ì–¸ì–´",
        cosmetic_title = "ì½”ìŠ¤ë©”í‹± ë³€ê²½", weapons = "ë¬´ê¸°", cosmetics = "ì½”ìŠ¤ë©”í‹±",
        filter_weapons = "ë¬´ê¸° ê²€ìƒ‰...", filter_cosmetics = "ì½”ìŠ¤ë©”í‹± ê²€ìƒ‰...",
        skin = "ìŠ¤í‚¨", wrap = "ëž©", rclick_hint = "ìš°í´ë¦­: ìŠ¤í‚¨ ì œê±°",
        unlock_all = "ì „ì²´ í•´ì œ", inverted = "ë°˜ì „", favorited = "ì¦ê²¨ì°¾ê¸°",
    },
    spanish = {
...
regionTab:AddInput("RegionSpoofValue", {
    ...
    Callback = function(val)
        config.profile.regionspoof.value = val or "Israel"
...
local WatermarkConnection = game:GetService('RunService').Heartbeat:Connect(function()
    ...
    if (tick() - FrameTimer) >= 1 then
        ...
        FrameTimer = tick()
        FrameCounter = 0
        Library:SetWatermark(('skidhub | %s fps'):format(
            math.floor(FPS)
        ))
    end
end)

Library:SetWatermark('skidhub')
...
-- settings tab addons (ThemeManager + SaveManager)
ThemeManager:SetLibrary(Library)
SaveManager:SetLibrary(Library)

SaveManager:IgnoreThemeSettings()

ThemeManager:SetFolder("ph4ntm")
SaveManager:SetFolder("ph4ntm")
...
if Ok then
    SaveManager:SetSubFolder(ProductInfo.Name)
end

SaveManager:BuildConfigSection(Tab6)
ThemeManager:ApplyToTab(Tab6)
